...
while(true)
    ...
    disp('     25 - Find circles with specific radius;');
    disp('     26 - Find round objects;');
    disp('     0 - Exit.');
    disp(' ');
    ...
    switch option
        ...
        case 25
            close all;
            min_r = input('Minimum radius: ');
            max_r = input('Maximum radius: ');
            [centers, radii, metric] = imfindcircles(image_gray,[min_r max_r]);
        ...
        case 26
            ...
            figure('Name','Round objects','NumberTitle','off');
            imshow(label2rgb(L, @jet, [.5 .5 .5])); % Jet for 64 colors
            hold on;
            for k = 1:length(B)
              boundary = B{k};
            ...
            threshold = 0.90; % metric = 4*pi*area/perimeter^2
            for k = 1:length(B) % loop over the boundaries
              ...
              text(boundary(1,2),boundary(1,1),metric_string,'Color','black','FontSize',14,'FontWeight','bold');
            end
